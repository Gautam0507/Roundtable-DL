...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.c_projection = nn.Linear(hidden_size, hidden_size, bias=True)
        self.att_projection = nn.Linear(hidden_size, hidden_size, bias=True)
        self.combined_output_projection = nn.Linear(hidden_size, hidden_size, bias=True)
        self.target_vocab_projection = nn.Linear(hidden_size, hidden_size, bias=True)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        rnn_output, (hn, cn) = self.encoder(x)
        seq_unpacked, lens_unpacked = pad_packed_sequence(rnn_output)
        enc_hiddens = torch.permute(seq_unpacked, (1, 0, 2))
        hn = torch.cat((hn), 1)
        cn = torch.cat(cn[0], cn[1])
