...
\end{lstlisting}
    ...
    \ifans{1. \textbf{What effect: }for every batch, those attention scores which have corresponding zero-padded embeddings are set to $-\infty$. This way, during the calculation of \textit{attention distributions} $\b\alpha_t$, the probabilities are calculated over scores with corresponding non-padded words whereas the scores with corresponding padded words are negligible. Finally, during the calculation of \textit{attention outputs} $A_t$, for every batch only the addition of those hidden states matter which are not multiplied by a probability close to $0$. \newline
    2. \textbf{Why necessary: }using masks in this way is an efficient way to determine the true \textit{attention distribution} that only involves the non-padded entries. Involving padded entries would result in false \textit{attention} representation.}
\end{parts}
