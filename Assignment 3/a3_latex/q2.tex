...
\begin{parts}
    ...
    \ifans{The dependency parser takes 2 steps to mark and remove each word from the stack. It takes in the word using the SHIFT and then another for marking the LEFT-ARC or the RIGHT-ARC reduce. Thus for $n$ words it would take $2n$ steps to parse the data.}
...
\input{dep_parses.tex}
 ...
 \ifans{\begin{enumerate}[i.]
		\item ``Love looks not with the eyes but with the mind.''
		\begin{itemize}
		...
		\end{itemize}
		\item ``There was an old man chasing off wild dogs with black fur.''
		\begin{itemize}
			\item \textbf{Error type}: Prepositional Phrase Attachment Error
			\item \textbf{Incorrect dependency}: chasing $\rightarrow$ fur
			\item \textbf{Correct dependency}: dogs $\rightarrow$ fur
		\end{itemize}
		\item ``Researchers are surprised by the unexpectedly good performance of GPT3 on various tasks.''
		\begin{itemize}
		...
		\end{itemize}
		\item ``I saw a tumbleweed crossing the street while eating my breakfast.''
		\begin{itemize}
...
\ifans{Part-of-speech tags are used as features in the parser to help to resolve syntactic ambiguities and improve the accuracy of the parsing. They provide information about the syntactic category and function of words in a sentence, which can help to assign different probabilities to different possible parses. They can also help to disambiguate words that have multiple possible tags, such as nouns and verbs, by using the context of other words in the sentence.}
